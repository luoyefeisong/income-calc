...
static void listener_cb(  struct evconnlistener *listener,   evutil_socket_t conn_sock,
						  struct sockaddr *client_addr,   int socklen,   void *user_data)
{
	XL_DEBUG(EN_PRINT_DEBUG, "listener_cb ...");
}
#define _WINSOCK_DEPRECATED_NO_WARNINGS 
void ping_cb(  evutil_socket_t sock,   short events,   void *user_data)
{
...
static int connect_peer(struct p2p_msg_device_info_t *pdevice_info)
{
	...
	while (--retry)
	{
		...
		if (-1 == set_sock_opt(sock, SO_REUSEADDR|SO_KEEPALIVE))
		{
...
void *process_punch_hole_cmd_cb(  void *arg)
{
	int sock = -1;
...
static void conn_read_cb(struct bufferevent *pbev,   void *user_data)
{
...
static int connect_server(struct p2p_transfer_client_t *ppt_client)
{
	...
	if (-1 == bind(sock, (const struct sockaddr *)&local_addr, addr_len))
	{
		...
		XL_DEBUG(EN_PRINT_ERROR, "call bind() failed, err: %s", strerror(dwErr));
		goto ERR;
	}
...
void exit_cb(  evutil_socket_t sock,   short events,   void *user_data)
{
...
struct p2p_transfer_client_t *init(int argc, char **argv)
{
	...
	if (-1 == configure_log(EN_PRINT_DEBUG, "./p2p_transfer_peer.log", 1))
	{
...
void user_cb(  evutil_socket_t tmp_sock,   short events,   void *user_data)
{
	XL_DEBUG(EN_PRINT_ERROR, "be called");
...
int main(int argc, char **argv)
{
	...
	if (WSAStartup(MAKEWORD(2, 2), &WSAData) != 0)
		printf("Windows sockets 2.2 startup error");
